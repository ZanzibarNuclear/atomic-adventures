...
import { axialToPixel, hexCorners, hexDistance } from "../composables/useHexGeometry.js";
...
import { hexPolygon, pointInHexPolygon } from "./hexPolygon.js";
...
function canReachInHex(hex, from, to, ctx, size) {
  return pathInHex(hex, from, to, ctx, size, pathClear) != null;
}
...
function reachableSharedBorderEntries(fromHex, toHex, fromPos, ctx, size) {
  ...
  const entries = [];
  for (const sample of sampleSharedEdge(edge, toHex, size)) {
    const approach = pathInHex(fromHex, fromPos, sample.edgePoint, travelCtx, size, pathClear);
...
function barrierMidpointStand(entryPoint, toHex, ctx, size, hexAtPoint) {
  ...
  const path = pathInHex(toHex, entryPoint, stand, travelCtx, size, pathClear);
...
function standReachableFromEntry({
  ...
  const path = pathInHex(toHex, entryPoint, target, travelCtx, size, pathClear);
  if (path) return path[path.length - 1];
  return null;
}
 ...
 */
export function findReachableBorderEntry({
  fromHex,
  toHex,
  fromPos,
  toPos,
 ...
 */
export function resolveDestinationStand({
  entryPoint,
  fromHex,
  toHex,
  fromPos,
...
export function resolveBlockedDeparture({
  fromHex,
  fromPos,
  toPos,
  walkPath,
