...
beat-wide authored actions, completion condition, effects, and the handoff to
the next beat or arc.

A **Scene** is one contextual move within a story beat. It attaches Zanzibar's
authored voice, choices, open-world area description, and revisit prose to a
location, event, or state-specific moment. A character may make one or more
scene-level moves before the active beat is complete. Multiple scenes may
belong to one story beat, and one location may host scenes from many beats.
...
active arc. Contextual choices come from the selected scene. Beat-wide authored
actions remain attached to the active story beat, so the progression state and
its durable guidance can remain stable while scenes change.
...
- The active scene's choices remain available on both first-view and revisit
  presentations.
...
Choices live on the selected `Scene`. They express the decisions available in
the particular location and circumstances that selected that scene. A change
of scene may change both prose and choices without advancing the StoryBeat.
...
card. It does not also move the player, and it leaves the current scene, beat,
and scene choices active. See [stage-views.md](stage-views.md).
