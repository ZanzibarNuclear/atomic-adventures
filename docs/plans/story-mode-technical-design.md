...
A scene is the prose presentation for a beat under specific circumstances. A
single story beat may have multiple scenes because the beat can look different
depending on where Zanzibar is standing, how he arrived, what time it is, what
flags are set, or whether the player has seen that scene before.
...
A choice is an authored player-facing action attached to a story beat.
...
Choices live on the `StoryBeat`, not on `Scene`. Scenes provide prose variants
for the beat. Choices are leaf nodes: they do not independently evaluate the
circumstances that selected the scene. If circumstances need substantially
different choices, that is a sign the story may need a different beat or an
explicit authored action generated by the controller for that beat.
...
2. beat-authored actions and choices;
...
- combining engine-provided possible actions with beat-authored actions;
- applying choices and story actions;
...
1. Choices live on `StoryBeat`, not on `Scene`. Scenes are prose variants.
